...
const store = usenonLinearPharmacokineticsStore();
const { chartOptions } = usenonLinearPharmacokineticsStore()
...
const decimal = computed({
  ...
  set: (newValue) => {
    ...
    updateOptions({ yaxis: { labels: { formatter: (value: number) => value.toFixed(newValue) } },
      tooltip: { y: { formatter: (value: number) => `${value.toFixed(newValue)} mg/mL` } }
    })
  }
})
